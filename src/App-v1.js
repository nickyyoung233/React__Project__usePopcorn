    ...
    <>
      ...
      <Main>
        ...
        <ContentBox>
          ...
          <MovieList movies={watched} cardType={2} />
        </ContentBox>
      </Main>
    </>
  );
...
function MovieList({ movies, cardType = 1 }) {
  return (
    <ul className="list">
      {movies?.map((movie) =>
        cardType === 1 ? (
          <MovieCard1 key={movie.imdbID} movie={movie} />
        ) : (
          <MovieCard2 key={movie.imdbID} movie={movie} />
        )
      )}
    </ul>
  );
  ...
  return (
    <div className="summary">
      ...
      <div>
        ...
        <InfoItem icon={"⭐️"} text={avgImdbRating} />
        <InfoItem icon={"🌟"} text={avgUserRating} />
        <InfoItem icon={"⏳"} text={`${avgRuntime} min`} />
      </div>
    </div>
  );
}
